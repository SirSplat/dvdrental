DROP TABLE IF EXISTS rental.temp_json;
CREATE TABLE rental.temp_json (val json);

TRUNCATE TABLE rental.temp_json;
INSERT INTO rental.temp_json (val) VALUES (
    '{
        "name": "Afghanistan",
        "alpha-2": "AF",
        "alpha-3": "AFG",
...
DROP TABLE IF EXISTS rental.country_source;
CREATE TABLE rental.country_source (
    name TEXT,
    alpha_2 TEXT,
    alpha_3 TEXT,
...
INSERT INTO rental.country_source (
    name,
    alpha_2,
    alpha_3,
    country_code
...
SELECT
    (val->>'name')::TEXT AS country_name,
    (val->>'alpha-2')::TEXT AS iso2_text,
    (val->>'alpha-3')::TEXT AS iso3_text,
    (val->>'country-code')::INTEGER AS country_code
FROM 
    rental.temp_json;
...
FROM
    rental.country AS c
    FULL OUTER JOIN rental.country_source AS cs ON (
        cs.name = c.country
    )
ORDER BY
    cs.name NULLS FIRST, country;
--    c.country NULLS FIRST, name;

BEGIN;
    -- Find everything that depends on rental.country
    DROP VIEW rental.customer_list;
    DROP VIEW rental.staff_list;
    DROP VIEW rental.sales_by_store;

    -- Fix current rental.country:
    ALTER TABLE rental.country ALTER COLUMN country SET DATA TYPE TEXT;

    -- rental.country.country has 21 records that do not have a match in rental.country_source.name
    UPDATE rental.country
    SET country = cs.name
    FROM rental.country_source AS cs
    WHERE
        country = 'Bolivia' AND
        cs.name = 'Bolivia, Plurinational State of';
    ...
    WHERE
        country = 'Brunei' AND
        cs.name = 'Brunei Darussalam';
        
    UPDATE rental.country
    SET country = cs.name
    FROM rental.country_source AS cs
    WHERE
        country = 'Congo, The Democratic Republic of the' AND
        cs.name = 'Congo, Democratic Republic of the';
    ...
    FROM rental.country_source AS cs
    WHERE
        country = 'Holy See (Vatican City State)' AND
        cs.name = 'Holy See';
    ...
    WHERE
        country = 'Iran' AND
        cs.name = 'Iran, Islamic Republic of';
        
    UPDATE rental.country
    SET country = cs.name
    FROM rental.country_source AS cs
    WHERE
        country = 'Kazakstan' AND
        cs.name = 'Kazakhstan';
    ...
    WHERE
        ...
        cs.name = 'Netherlands, Kingdom of the';
        
    UPDATE rental.country
    SET country = cs.name
    FROM rental.country_source AS cs
    WHERE
        country = 'North Korea' AND
        cs.name = 'Korea, Republic of';
    ...
    FROM rental.country_source AS cs
    WHERE
        country = 'South Korea' AND
        cs.name = 'Korea, Democratic People''s Republic of';
    ...
    WHERE
        country = 'Taiwan' AND
        cs.name = 'Taiwan, Province of China';
        
    UPDATE rental.country
    SET country = cs.name
    FROM rental.country_source AS cs
    WHERE
        country = 'Tanzania' AND
        cs.name = 'Tanzania, United Republic of';
    ...
    WHERE
        ...
        cs.name = 'United Kingdom of Great Britain and Northern Ireland';
    ...
    WHERE
        ...
        cs.name = 'United States of America';
    ...
    WHERE
        ...
        cs.name = 'Venezuela, Bolivarian Republic of';
    ...
    FROM rental.country_source AS cs
    WHERE
        country = 'Virgin Islands, U.S.' AND
        cs.name = 'Virgin Islands (U.S.)';

    -- Add the new columns to rental.country
    ALTER TABLE rental.country ADD COLUMN iso2 TEXT;
    ALTER TABLE rental.country ADD COLUMN iso3 TEXT;
    ALTER TABLE rental.country ADD COLUMN country_code INTEGER;
    
    -- UPSERT everything from rental.country_source into rental.country
    MERGE INTO rental.country AS c
    USING rental.country_source AS cs ON cs.name = c.country
    WHEN NOT MATCHED THEN
        INSERT ( country, iso2, iso3, country_code ) VALUES ( cs.name, cs.alpha_2, cs.alpha_3, cs.country_code )
    WHEN MATCHED THEN
        UPDATE SET 
            country = cs.name,
            iso2 = cs.alpha_2,
    ...
    -- 'Yugoslavia' -- Dissolution in 1992 to Slovenia, Croatia, Serbia ...
    --  So i'll UPDATE all rental.city.country_id = 108 to ( SELECT country_id FROM rental.country WHERE country = 'Serbia'; ) I checked Google for where the cities were = Serbia. So there you have it
    UPDATE 
        rental.city
    SET 
        country_id = (
    ...
    DELETE FROM rental.country WHERE country_id = 108;

    -- Set the new columns to NOT NULL
    ALTER TABLE rental.country ALTER COLUMN iso2 SET NOT NULL;
    ALTER TABLE rental.country ALTER COLUMN iso3 SET NOT NULL;
    ALTER TABLE rental.country ALTER COLUMN country_code SET NOT NULL;

    -- Re-create everything that depends on rental.country
    CREATE OR REPLACE VIEW rental.sales_by_store
    ...
    CREATE OR REPLACE VIEW rental.staff_list
    ...
    CREATE OR REPLACE VIEW rental.customer_list
...
SELECT country, count(*) FROM rental.country GROUP BY country ORDER BY 2 DESC;
SELECT * FROM rental.city WHERE country_id = 108;
